<?php
// Reset Vouchers Tables via Web
// Access this file in browser: http://localhost:port/fix_vouchers_web.php
...
try {
    ...
    echo "<h1>Database Voucher Reset</h1>";
    echo "<pre>";
    ...
    // 2. Clear references in orders table (optional but safe)
    // We don't drop orders table, just clear the voucher link
    try {
        $pdo->exec("UPDATE `orders` SET `voucher_id` = NULL");
        echo "Cleared voucher references in orders table.\n";
    } catch (PDOException $e) {
        echo "Note: Could not update orders table (maybe column missing?): " . $e->getMessage() . "\n";
    }

    // 3. Drop tables
    $pdo->exec("DROP TABLE IF EXISTS `voucher_usage`");
    echo "Dropped table 'voucher_usage'.\n";

    $pdo->exec("DROP TABLE IF EXISTS `vouchers`");
    echo "Dropped table 'vouchers'.\n";

    // 4. Re-enable Foreign Key Checks
    $pdo->exec("SET FOREIGN_KEY_CHECKS = 1");
    echo "Foreign Key Checks enabled.\n";

    // 5. Create vouchers table (Full Schema)
    ...
    echo "Created table 'vouchers'.\n";

    // 6. Create voucher_usage table
    $sqlUsage = "CREATE TABLE `voucher_usage` (
      `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    $pdo->exec($sqlUsage);
    echo "Created table 'voucher_usage'.\n";

    // 7. Ensure orders table has columns (Idempotent check)
    ...
    if (!$stmt->fetch()) {
        ...
        $pdo->exec($sqlAlter);
        echo "Altered table 'orders' to add voucher columns.\n";
    } else {
        echo "Table 'orders' already has voucher columns.\n";
    }
